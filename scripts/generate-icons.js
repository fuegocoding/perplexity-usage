...
function drawGauge(x, y, s) {
  ...
  // ---- ARC (270 degree gauge, 90 degree gap at bottom) ----
  ...
  // Soft glow just outside arc band
  ...
  const needleHalfW = Math.max(0.8, s / 60);
...
function drawIcon(x, y, s, dotColor) {
  ...
  if (dot[3] > 0) return compositeOver(dot, gauge);
  return gauge;
}
...
for (const sz of SIZES) {
  fs.writeFileSync(`icon${sz}.png`, createPNG(sz, (x, y, s) => drawIcon(x, y, s, null)));
  fs.writeFileSync(`icon${sz}-green.png`, createPNG(sz, (x, y, s) => drawIcon(x, y, s, GREEN)));
  fs.writeFileSync(`icon${sz}-red.png`, createPNG(sz, (x, y, s) => drawIcon(x, y, s, RED)));
}
